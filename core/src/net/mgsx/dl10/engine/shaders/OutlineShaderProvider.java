package net.mgsx.dl10.engine.shaders;
...
public class OutlineShaderProvider extends PBRDepthShaderProvider
{
	...
	private static Config createConfig(int numBones) {
		...
		config.vertexShader = Gdx.files.classpath("net/mgsx/dl10/engine/shaders/gltf-outline-ext.vs.glsl").readString();
		config.fragmentShader = Gdx.files.classpath("net/mgsx/dl10/engine/shaders/gltf-outline-ext.fs.glsl").readString();
		return config;
	}
